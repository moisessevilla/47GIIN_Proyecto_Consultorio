<template>
    <div class="contenido">
      ...
      <form @submit.prevent="handleSubmit" class="formulario">
        <div class="form-group">
          ...
          <input id="ncolegiado" v-model="form.ncolegiado" type="text" minlength="6" maxlength="8" placeholder="Introduce número de colegiado" required />
        </div>
        <div class="form-group">
          <label for="nombre">Nombre:*</label>
          <input id="nombre" v-model="form.nombre" type="text" minlength="3" maxlength="100" placeholder="Introduce nombre del colegiado" required />
        </div>
        <div class="form-group">
          <label for="especialidad">Especialidad:*</label>
          <input id="especialidad" v-model="form.especialidad" type="text" minlength="3" maxlength="100" placeholder="Introduce la especialidad" required />
        </div>
        <div class="form-group">
  ...
  export default {
    ...
    validateEmail(event) {
      ...
      async fetchMedicos() {
        try {
          const response = await axios.get("/citas/medicos/");
      ...
      async handleSubmit() {
        try {
          if (this.form.id_medico) {
            ...
            await axios.put(`/citas/medicos/${this.form.id_medico}/`, updateData, {
            ...
            // Crear médico
            await axios.post("/citas/medicos/", this.form, {
              headers: {
          ...
          console.error("Error al guardar el médico:", error.response?.data || error.message);
          ...
          if (error.response?.data) {
            ...
            alert("Hubo un error al guardar el médico.");
          }
        }
      },
      ...
      async deleteMedico(id_medico) {
        if (confirm("¿Estás seguro de eliminar este médico?")) {
          try {
            await axios.delete(`/citas/medicos/${id_medico}/`, {
              headers: {
                'Content-Type': 'application/json',
              },
            });
